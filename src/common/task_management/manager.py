...
class TaskManager:
    ...
    def update_tasks_in_state(
        state: Dict[str, Any], task_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for data in task_data:
                ...
                if "id" in data and data["id"] in existing_ids:
                    ...
                    # Create new task
                    new_task = Task(
                        content=data["content"], status=status, priority=priority
                    )
                    if "id" in data:
                        new_task.id = data["id"]
            ...
            return {"success": False, "error": f"Task management failed: {str(e)}"}
