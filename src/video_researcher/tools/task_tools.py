...
def task_manager(state: Dict[str, Any], tasks: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    """
    ...
    for i, task in enumerate(tasks):
        if not isinstance(task, dict):
            return {
                "success": False,
                "error": f"Task item {i} must be a dictionary"
            }
        ...
        if "status" in task and task["status"] not in ["pending", "in_progress", "completed"]:
            return {
                "success": False,
    ...
    if result["success"]:
        ...
        result["message"] = f"Task list updated successfully. {', '.join(message_parts)}. {status_summary} {completion}"
